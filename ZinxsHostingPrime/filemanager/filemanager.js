...
function fetchDataInit() {
    endpoint += '&ftp&opt&minecraft';
    // send GET request to backend to get list of files
    fetch('http://localhost:8091/zinxshosting/backend/getConsoleDirectory/'+endpoint)
...
function postFileData(){
  filepath = endpoint.replaceAll('&ftp&opt&','');
  fetch('http://localhost:8091/zinxshosting/backend/postFtpFile/'+filepath,{
    method: 'POST',
    headers:{
...
function getContent(filepath){
  filepath = filepath.replaceAll('&ftp&opt&','');
  fetch('http://localhost:8091/zinxshosting/backend/getFtpFile/'+filepath)
  .then(response => response.text())
  .then(data => {
    data = data.replaceAll('[','');
    data = data.replaceAll(']','');
...
function displayFileContent(files) {
  ...
  function createBackButton(filesElement){
    if(!(endpoint.normalize() === "&ftp&opt&minecraft".normalize())){
